...
describe("Invalid Tool Call Handler", () => {
  test("createInvalidToolCallTool returns a valid tool", () => {
    ...
    expect(errorTool.description).toBe("Internal tool - surfaces validation errors for invalid tool calls")
  ...
  test("error tool formats message correctly", async () => {
    ...
    }, { toolCallId: "test-123", messages: [], abortSignal: undefined as unknown as AbortSignal })
  ...
  test("error tool includes all provided context", async () => {
    ...
    }, { toolCallId: "test-456", messages: [], abortSignal: undefined as unknown as AbortSignal })
...
describe("Tool Call Repair Function", () => {
  ...
  test("repair function preserves original tool call info in args", async () => {
    ...
    const result = await repair({
      ...
      tools: {},
      parameterSchema: () => ({}),
      error: new Error("filePath: Required; oldString: Required; newString: Required"),
    })
...
describe("Tool Execution Error Wrapping", () => {
  ...
  function wrapExecute(originalExecute: (args: unknown, context: unknown) => Promise<string>) {
...
describe("prepareTools", () => {
  ...
  function prepareTools(tools: Record<string, { execute?: (args: unknown, context: unknown) => Promise<string> }>) {
  ...
  test("prepareTools wraps execute to catch errors", async () => {
    ...
    const prepared = prepareTools(tools) as Record<string, { execute: (args: unknown, context: unknown) => Promise<string> }>
