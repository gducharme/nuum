...
import { createInMemoryStorage, createStorage, initializeDefaultEntries, ConflictError, getRawConnection } from "./index"
...
async function verify() {
  ...
  const msg1 = { id: Identifier.ascending("message"), type: "user", content: "Hello", tokenEstimate: 10, createdAt: new Date().toISOString() }
  const msg2 = { id: Identifier.ascending("message"), type: "assistant", content: "Hi there", tokenEstimate: 15, createdAt: new Date().toISOString() }
  const msg3 = { id: Identifier.ascending("message"), type: "user", content: "How are you?", tokenEstimate: 20, createdAt: new Date().toISOString() }
  ...
  check("1. estimateUncompactedTokens (no summaries)", uncompacted === 45, `got ${uncompacted}, expected 45`)
  ...
  check("1b. estimateUncompactedTokens (after summary)", uncompacted === 20, `got ${uncompacted}, expected 20 (only msg3)`)

  // 4. Messages ordered by ULID
  const messages = await storage.temporal.getMessages()
  const orderedCorrectly = messages[0].id === msg1.id && messages[1].id === msg2.id && messages[2].id === msg3.id
  ...
  check("5. getSummaries(order) filters correctly", order1Summaries.length === 1 && order1Summaries[0].orderNum === 1)
  ...
  const updated = await storage.ltm.update("test-entry", "Updated content", 1, "ltm-consolidate")
  check("2a. CAS update with correct version succeeds", updated.version === 2)
  check("8c. updatedBy changed after update", updated.updatedBy === "ltm-consolidate")
  ...
  check("2b. CAS update with wrong version throws ConflictError", casErrorThrown)

  // 6. glob() handles path patterns
  await storage.ltm.create({ slug: "knowledge", parentSlug: null, title: "Knowledge", body: "Root", createdBy: "main" })
  await storage.ltm.create({ slug: "react", parentSlug: "knowledge", title: "React", body: "React docs", createdBy: "main" })
  await storage.ltm.create({ slug: "hooks", parentSlug: "react", title: "Hooks", body: "Hooks docs", createdBy: "main" })

  const globResult = await storage.ltm.glob("/knowledge/**")
  check("6. glob('/knowledge/**') matches subtree", globResult.length >= 2, `got ${globResult.length} entries`)
  ...
  const tasks = [
    ...
    { id: "task-4", content: "Fourth task", status: "blocked" as const, blockedReason: "Waiting for X" },
  ]

  await storage.present.setTasks(tasks)
  const state = await storage.present.get()

  const tasksMatch = state.tasks.length === 4 &&
  ...
  check("3b. /identity has sensible content", identity?.body.includes("Identity") ?? false)
  check("3c. /behavior created", behavior !== null)
  check("3d. /behavior has sensible content", behavior?.body.includes("Behavior") ?? false)
